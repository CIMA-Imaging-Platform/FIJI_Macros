...
*/
  ...
  * - Save a results csv as well as a xls
  * - Fixes regarding batch processing of a directory of images
*/

var r=0.502, d=20, D=300, thFac=0.98, mic="Microns", type="One Image", auto=true, dir="", filename="",mode="End",exedges=false;
...
if(mic=="Pixel"){
	...
	Dialog.addNumber("AutoThreshold factor:", thFac);
	Dialog.show();
	r=1;
	d=Dialog.getNumber();
	D=Dialog.getNumber();
	thFac=Dialog.getNumber();
}
else{	
	Dialog.create("-Calibration-")
...
if(type=="One Image"){
	...
	Adiposoft(OutDir,dir,name,auto,r,d,D,thFac,type);
	setBatchMode(false);
	if(auto==1){
...
if(type=="A Directory"){
	...
	AdiposoftDir(OutDir,InDir,auto,r,d,D,thFac,type);}
...
if(type=="A list of nested Directories - Batch Mode"){
	...
	for (i=0; i<L; i++)
	{
		...
		if(File.isDirectory(InDir+list[i]))
		{
			//print(OutDir+list[i]);
			dir=InDir+list[i];
			AdiposoftDir(OutDir,dir,auto,r,d,D,thFac,type);
		}
...
function AdiposoftDir (OutDir,InDir,auto,r,d,D,thFac,type) {
...
for (i=0; i<L; i++)
{
	if(endsWith(list[i],".tif")||endsWith(list[i],".jpg"))
	{
		...
		//print(AuxDir);
		//waitForUser("aa");
		Adiposoft(AuxDir,InDir,list[i],auto,r,d,D,thFac,type);
		roiManager("reset");
		run("Clear Results");
		close();
		setBatchMode(false); 
...
selectWindow("Total");
IJ.renameResults("Results");
saveAs("Results", OutDir+name1+".csv");
...
function Adiposoft(output,dir,filename,auto,r,d,D,thFac,type) {
	//eval("js", "IJ.getInstance().show()");
	//eval("js","RoiManager.getInstance().hide()"); 
	run("Clear Results");
	roiManager("reset");
	...
	if(nResults==0){//check if no adypocites detected, then inform and close - continue if directory
		...
		//waitForUser("a");
		saveAs("Results", output+MyTitle+".csv");
		...
		if(auto==1){
			...
			//Open the toolbar
			showMessage("Adiposoft 1.17","Succesfully processed. Results stored in '" + OutDir+"'");
			close;
			selectWindow("Results");
			run("Close");
			run("Collect Garbage");
...
function Edition(){
	...
	Dialog.create("-Adiposoft_1.17- Manual Edition")
