...
// volume and intensity measurements for 2 nuclear proteins and 1 cytoplasmic protein

var thProjection=22, thCell=20, thNucl=40, cNuclPr1=1, cNuclPr2=2, cCytoPr=3, cDAPI=4;			
...
macro "MP14 Action Tool 1 - Cf00T2d15IT6d10m"{
	...
	Dialog.addNumber("Nuclear protein 1", cNuclPr1);	
	Dialog.addNumber("Nuclear protein 2", cNuclPr2);	
	Dialog.addNumber("Cytoplasmic protein", cCytoPr);
	Dialog.addNumber("DAPI", cDAPI);		
	Dialog.show();	
	cNuclPr1= Dialog.getNumber();
	cNuclPr2= Dialog.getNumber();
	cCytoPr= Dialog.getNumber();
	cDAPI= Dialog.getNumber();
	
	//setBatchMode(true);
	print(name);
	ifnuclcyto3d("-","-",name,cNuclPr1,cNuclPr2,cCytoPr,cDAPI);
	setBatchMode(false);
	showMessage("Done!");
...
macro "MP14 Action Tool 2 - C00fT0b11DT9b09iTcb09r"{
	...
	for (j=0; j<L; j++)
	{
		if(endsWith(list[j],"czi")){
			...
			print(name);
			//setBatchMode(true);
			ifnuclcyto3d(InDir,InDir,list[j],cNuclPr1,cNuclPr2,cCytoPr,cDAPI);
			setBatchMode(false);
			}
	}
	showMessage("Done!");
...
function ifnuclcyto3d(output,InDir,name,cNuclPr1,cNuclPr2,cCytoPr,cDAPI)
{
...
Stack.setChannel(1);	// nuclear protein 1
run("Red");
Stack.setChannel(2);	// nuclear protein 2
run("Yellow");
Stack.setChannel(3);	// cytoplasmic protein
run("Green");
Stack.setChannel(4);	// DAPI
run("Blue");
...
run("Split Channels");
// Nuclear protein 1:
selectWindow("C"+cNuclPr1+"-orig");
run("Subtract Background...", "rolling=50 stack");
rename("NuclPr1");
// Nuclear protein 2:
selectWindow("C"+cNuclPr2+"-orig");
run("Subtract Background...", "rolling=50 stack");
rename("NuclPr2");
// Cytoplasmic protein:
...
run("Merge Channels...", "c1=NuclPr1 c2=NuclPr2 c3=CytoPr c4=DAPI create keep");
Stack.setDisplayMode("composite");
Stack.setChannel(1);	// nuclear protein 1
run("Red");
Stack.setChannel(2);	// nuclear protein 2
...
// If no proteins in channel 1 or 2, set display range to cytoplasmic protein:
if (max1<100) {
	Stack.setChannel(1);
	setMinAndMax(min1, max3);
}
if (max2<100) {
	Stack.setChannel(2);
	setMinAndMax(min2, max3);
}
...
InuclPr1=newArray(nCells);
InuclPr2=newArray(nCells);
	...
	for (j=0;j<slices;j++){				
		...
		type = selectionType();
	  	if (type!=-1)
	  	{
	  		...
	  		Ii=getResult("Mean",0);
	  		Aacc=Aacc+Ai;
	  		Iacc1=Iacc1+Ii*Ai;	// weigh each slice intensity with the area of the ROI
	  		// Nuclear protein 2:
	  		selectWindow("NuclPr2");
	  		setSlice(j+1);
	  		run("Restore Selection");
	  		run("Measure");
	  		Ai=getResult("Area",1);
	  		Ii=getResult("Mean",1);	  		
	  		Iacc2=Iacc2+Ii*Ai;	// weigh each slice intensity with the area of the ROI
	  	}
	}
	Vnucl[i]=Aacc*depth;
	InuclPr1[i]=Iacc1/Aacc;
	InuclPr2[i]=Iacc2/Aacc;
	selectWindow("aa");
	close();
...
for(j=0;j<nCells;j++){
	...
	setResult("Iavg nucl prot 1", i, InuclPr1[j]);
	setResult("Iavg nucl prot 2", i, InuclPr2[j]);
