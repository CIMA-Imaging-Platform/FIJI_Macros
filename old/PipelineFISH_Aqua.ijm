...
var nDAPI=1, nGreen=2, nRed=2, nAqua=2, diskRad=12, minG=0, maxG=120, minR=0, maxR=120, minA=0, maxA=120;
...
//--AQUA: enhance each aqua channel and combine them by avg projection

for (i = 1; i <= nAqua; i++) {
	ch = nDAPI+nRed+nGreen+i;
	selectWindow("orig");	
	run("Duplicate...", "title=a"+i+" duplicate channels="+ch);
	wait(50);
	//--Remove out-of-focus light and keep only defined FISH spots using a white top-hat filter
	  //diskRad=12;
	run("Morphological Filters", "operation=[White Top Hat] element=Disk radius="+diskRad);
...
if(nAqua>1) {
	for (i = 1; i < nAqua; i++) {
		i2 = i+1;
		run("Concatenate...", "  image1=a1 image2=a"+i2+" image3=[-- None --]");
		rename("a1");
	}
	run("Z Project...", "projection=[Average Intensity]");
	rename("aqua");
...
run("Brightness/Contrast...");
waitForUser("Revise el ajuste de contraste para el aqua y modifique el minimo o maximo si fuese necesario. Pulse OK al finalizar.");
run("Apply LUT");


//--MERGE THE FOUR CHANNELS:

run("Merge Channels...", "c1=red c2=green c3=DAPI c5=aqua");
rename("final");
selectWindow("orig");
close();
selectWindow("final");
